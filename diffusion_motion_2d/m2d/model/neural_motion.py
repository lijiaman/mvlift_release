...
class NeuralMotionField(nn.Module):
    def __init__(self):
        ...
        if bandwidth != 0:
            self.positional_encoding = PositionalEncoding(bandwidth)
            embedding_dim = bandwidth * 2
        else:
            embedding_dim = 1
        ...
        self.global_linear = nn.Sequential(nn.Linear(hidden_neuron, global_output))

    def forward(self, t, z_l=None, z_g=None):
        if self.bandwidth != 0 and self.siren is False:
