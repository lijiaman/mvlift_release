...
class MultiViewDecoder(nn.Module):
    def __init__(
        ...
        self.view_position_vec = nn.Embedding.from_pretrained(
            get_sinusoid_encoding_table(num_views+1, d_model, padding_idx=0),
            freeze=True)

        # Cross-attention for different views 
        self.cross_view_layer_stack = nn.ModuleList([DecoderLayer(d_model, n_head, d_k, d_v)
            for _ in range(n_layers)])
        # for cross_view_layer in self.cross_view_layer_stack:
        #     zero_module(cross_view_layer)
    ...
    def forward(self, decoder_input, padding_mask, decoder_pos_vec, obj_embedding=None):
        # decoder_input: (BS*K) X D X T 
        # padding_mask: (BS*K) X 1 X T
        # decoder_pos_vec: (BS*K) X 1 X T
        # obj_embedding: (BS*K) X 1 X D

        dec_self_attn_list = []
        dec_cross_view_attn_list = [] 

        padding_mask = padding_mask.squeeze(1) # (BS*K) X T
        bs = padding_mask.shape[0]//self.num_views 
        seq_len = padding_mask.shape[1] 
        cross_view_padding_mask = padding_mask.reshape(bs, self.num_views, -1) # BS X K X T 
        cross_view_padding_mask = cross_view_padding_mask.transpose(1, \
                            2).reshape(bs*seq_len, self.num_views) # (BS*T) X K 

        decoder_pos_vec = decoder_pos_vec.squeeze(1) # (BS*K) X T

        input_embedding = self.start_conv(decoder_input)  # (BS*K) X D X T
        input_embedding = input_embedding.transpose(1, 2) # (BS*K) X T X D
        if obj_embedding is not None:
            new_input_embedding = torch.cat((obj_embedding, input_embedding), dim=1) 
            # (BS*K) X (T+1) X D 
        else:
            new_input_embedding = input_embedding

        pos_embedding = self.position_vec(decoder_pos_vec) # (BS*K) X T X D
        
        # Time mask is same for all blocks, while padding mask differ according to the position of block
        if self.use_full_attention:
            time_mask = None
            cross_view_time_mask = None 
        else:
            time_mask = get_subsequent_mask(decoder_pos_vec) # (BS*K) X T X T 
            # (BS*K) X T X T (Prev steps are 0, later 1)

        # Get position vec for position-wise embedding
        view_pos_vec = torch.arange(self.num_views)+1 # K
        view_pos_vec = view_pos_vec[None, :, None].to(decoder_input.device).repeat(pos_embedding.shape[0]//self.num_views, \
                    1, pos_embedding.shape[1]) # BS X K X T 
        view_pos_embedding = self.view_position_vec(view_pos_vec) # BS X K X T X D 

        dec_output = new_input_embedding + pos_embedding # (BS*K) X T X D
        layer_idx = 0 
        for dec_layer in self.layer_stack:
            dec_output, dec_self_attn = dec_layer(
                dec_output, # (BS*K) X T X D
                self_attn_time_mask=time_mask, # None 
                self_attn_padding_mask=padding_mask) # (BS*K) X T

            # Apply cross-view attention 
            dec_output = dec_output.reshape(bs, self.num_views, seq_len, -1) # BS X K X T X D 
            dec_output += view_pos_embedding # BS X K X T X D 
            dec_output = dec_output.transpose(1, 2).reshape(bs*seq_len, self.num_views, -1) # (BS*T) X K X D 
            dec_output, dec_cross_view_attn = self.cross_view_layer_stack[layer_idx](
                dec_output, # (BS*T) X K X D 
                self_attn_time_mask=None, # None 
                self_attn_padding_mask=cross_view_padding_mask) # (BS*T) X K  

            # Convert the dimension of dec_output
            dec_output = dec_output.reshape(bs, seq_len, self.num_views, -1) # BS X T X K X D 
            dec_output = dec_output.transpose(1, 2).reshape(bs*self.num_views, seq_len, -1) # (BS*K) X T X D  

            dec_self_attn_list += [dec_self_attn]
            dec_cross_view_attn_list += [dec_cross_view_attn]  
